...
public class SyncPatientChartAction extends ActionSupport {
    ...
    public String getChartData() {
        ...
        chartUtil.executeUpdate("DROP TABLE IF EXISTS ps");
        
        query = "CREATE TEMPORARY TABLE ps AS SELECT DISTINCT facility_id, patient_id, time_stamp FROM patient WHERE facility_id = " + facilityId + " AND time_stamp >= DATEADD('MONTH', -8, CURDATE()) AND time_stamp <= DATEADD('MONTH', -1, CURDATE())";
        ...
        for (int i = 0; i <= monthsBetween; i++) {					
            ...
            query = "SELECT COUNT(*) AS count FROM ps WHERE MONTH(time_stamp) = " + month + "  AND YEAR(time_stamp) = " + year; 
            total = chartUtil.getCount(query);
            data.add(total);
            
                    System.out.println("Month...."+ month + " value...." + total);
            categories.add(periodLabel);
        }
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("name", "Enrolment");
        map.put("data", data);
        series.add(map);
