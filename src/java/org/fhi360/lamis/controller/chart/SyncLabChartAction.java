...
public class SyncLabChartAction extends ActionSupport {
    ...
    public String getChartData() {
        ...
        chartUtil.executeUpdate("DROP TABLE IF EXISTS lb");
        
        query = "CREATE TEMPORARY TABLE lb AS SELECT DISTINCT facility_id, patient_id, date_reported, time_stamp FROM laboratory WHERE facility_id = " + facilityId + " AND time_stamp >= DATEADD('MONTH', -8, CURDATE()) AND time_stamp <= DATEADD('MONTH', -1, CURDATE())";
        ...
        for (int i = 0; i <= monthsBetween; i++) {					
            ...
            query = "SELECT COUNT(*) AS count FROM lb WHERE MONTH(time_stamp) = " + month + "  AND YEAR(time_stamp) = " + year; 
