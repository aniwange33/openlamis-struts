 ...
 * @author user10
 */
public class HtsSettingSummaryChartService {

    private final static JdbcTemplate jdbcTemplate = ContextProvider.getBean(JdbcTemplate.class);
    private final static TransactionTemplate transactionTemplate = ContextProvider.getBean(TransactionTemplate.class);

    public HtsSettingSummaryChartService() {
    ...
    public Map<String, Integer> chartData(long ipId, long stateId, long lgaId, long facilityId, String dataElementIds, Date reportingDateBegin, Date reportingDateEnd) {
        ...
        jdbcTemplate.query(query, resultSet -> {
            while (resultSet.next()) {
                ...
                if (dataElementId == 104) {
                    map.put("value1", resultSet.getInt("value"));  //CT
                }
                if (dataElementId == 105) {
                    map.put("value2", resultSet.getInt("value"));  //TB
                }
                if (dataElementId == 106) {
                    map.put("value3", resultSet.getInt("value"));  //STI
                }
                if (dataElementId == 107) {
                    map.put("value4", resultSet.getInt("value"));  //OPD
                }
                if (dataElementId == 108) {
                    map.put("value5", resultSet.getInt("value"));  //Ward
                }
                if (dataElementId == 109) {
                    map.put("value6", resultSet.getInt("value"));  //Community
                }
                if (dataElementId == 110) {
                    map.put("value7", resultSet.getInt("value"));  //Stand alone
                }
                if (dataElementId == 111) {
