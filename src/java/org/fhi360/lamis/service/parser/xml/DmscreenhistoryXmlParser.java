...
public class DmscreenhistoryXmlParser extends DefaultHandler {
    ...
    public DmscreenhistoryXmlParser() {
    }
            ...
            DefaultHandler defaultHandler = new DefaultHandler() {
                ...
                public void characters(char[] chars, int start, int length) throws SAXException {
                    ...
                    if (idUUIDTag.equals("open")) {
                        dmscreenhistory.setUuid(new String(chars, start, length));
                    }
                }
                    ...
                    if (element.equalsIgnoreCase("dmscreenhistory")) {
                        ...
                        if (!skipRecord) {Long patientId = ServerIDProvider.getPatientServerId(hospitalNum, facilityId);
                            if (patientId != null) {
                                ...
                                Long id = ServerIDProvider.getScreenId("tbscreenhistory", dmscreenhistory.getDateVisit(),
                                        dmscreenhistory.getDescription(), hospitalNum, facilityId);
                                if (id != null) {
                                    dmscreenhistory.setHistoryId(id);
                                    DmscreenhistoryDAO.update(dmscreenhistory);
                                } else {
                                    DmscreenhistoryDAO.save(dmscreenhistory);
                                }
                            }
                        }
                    }
