...
public class EacJsonParser {
    ...
    public void parserJson(String table, String content) {
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                long patientId = new PatientJDBC().getPatientId(eac.getFacilityId(), eac.getPatient().getHospitalNum());
                Patient patient = new Patient();
                patient.setPatientId(patientId);

                eac.setPatient(patient);

                long devolveId = new EacJDBC().getEacId(eac.getFacilityId(), patientId, dateFormat.format(eac.getDateEac1()));
                if (devolveId == 0L) {
                    EacDAO.save(eac);
                } else {
                    eac.setEacId(devolveId);
                    EacDAO.update(eac);
                }
    ...
    private static Eac getObject(String content) throws JAXBException, JsonParseException, JsonMappingException, IOException {
        Eac eac = new Eac();
        try {
